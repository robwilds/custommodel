...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f custommodel-share
	call %MVN_EXEC% clean package -pl custommodel-share,custommodel-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill custommodel-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f custommodel-acs
	call %MVN_EXEC% clean package -pl custommodel-integration-tests,custommodel-platform,custommodel-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl custommodel-platform,custommodel-integration-tests,custommodel-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl custommodel-platform,custommodel-integration-tests
EXIT /B 0
:purge
